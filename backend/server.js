...
app.get("/api/lines/:lineId/stations", async (req, res) => {
  ...
  try {
    // First, verify the line exists in our list
    const lines = [
      { id: "AIR", name: "Airport Line" },
      { id: "CHE", name: "Chestnut Hill East Line" },
      { id: "CHW", name: "Chestnut Hill West Line" },
    ...
    const lineExists = lines.some((line) => line.id === lineId);
    if (!lineExists) {
      return res.status(404).json({ error: "Line not found" });
    }

    // Try to get stations from SEPTA API, but this is currently failing
    // Let's immediately use our fallback data
    if (stationData[lineId]) {
      console.log(`Using fallback data for ${lineId}`);
      return res.json({
        stations: stationData[lineId],
        route: stationData[lineId].map((station) => ({
          lat: station.lat,
          lng: station.lng,
        })),
      });
      ...
      return res
        .status(404)
        .json({ error: "No station data available for this line" });
    }
  } catch (error) {
    console.error("Error fetching stations:", error);

    // Access lineId from the function parameters inside the catch block
    if (stationData[lineId]) {
      console.log(`Using fallback data for ${lineId} after error`);
      return res.json({
        stations: stationData[lineId],
        route: stationData[lineId].map((station) => ({
          lat: station.lat,
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch stations data", details: error.message });
  }
});
